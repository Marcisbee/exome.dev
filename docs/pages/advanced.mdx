...
Every time action is called for specific instance, it triggers update event for it automatically. But sometimes when performance is on the line and updating UI so much may bring only harm. So there is a way to bypass actions.
